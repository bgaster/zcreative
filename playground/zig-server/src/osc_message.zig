...
pub const OscMessage = struct {
    ...
    pub fn init(address: []const u8, arguments: []const OscArgument) Self {
        return . {
            .address = address,
            .arguments = arguments,
        };
    ...
    pub fn decode(buffer: []u8, allocator: Allocator) !Self {
        var stream = std.io.fixedBufferStream(buffer);
        ...
        return init(
            try allocator.dupe(u8, address[0..]),
            try allocator.dupe(OscArgument, arguments.items)
        );
    }
};
